...
static void drawmenu(void) {
  ...
  if (lines > 0) {
    ...
    for (item = curr; item != next; item = item->right)
      ...
      x = drawitem(item, x, 0,
                   MIN(TEXTWM(item->text), mw - x - TEXTW(symbol_2) - rpad));
#elif PANGO_PATCH
      x = drawitem(item, x, 0,
                   MIN(TEXTWM(item->text), mw - x - TEXTW(">") - rpad));
#elif SYMBOLS_PATCH
      x = drawitem(item, x, 0,
                   MIN(TEXTW(item->text), mw - x - TEXTW(symbol_2) - rpad));
#else
      x = drawitem(item, x, 0,
                   MIN(TEXTW(item->text), mw - x - TEXTW(">") - rpad));
#endif // PANGO_PATCH
    if (next) {
#if SYMBOLS_PATCH
      w = TEXTW(symbol_2);
